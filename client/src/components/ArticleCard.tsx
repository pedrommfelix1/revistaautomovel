...
export function ArticleCard({ article, featured = false, index = 0 }: { article: CardArticle; featured?: boolean; index?: number }) {
  return (
    <article className={`group ${featured ? "featured-card" : "article-card"}`}>
      ...
      <div className={`${featured ? "pt-5" : "border-t border-black pt-4"}`}>
        ...
        {article.deck && <p className={`${featured ? "max-w-2xl text-base sm:text-lg" : "text-sm"} mt-4 leading-relaxed text-neutral-600`}>{article.deck}</p>}
        <div className="mt-5 flex items-center justify-between gap-4 border-t border-black/20 pt-3 text-[10px] font-bold uppercase tracking-[0.11em] text-neutral-600">
