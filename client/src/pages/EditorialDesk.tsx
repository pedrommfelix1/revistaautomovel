...
function statusBadge(article: { status: "draft" | "scheduled" | "published"; scheduledAt: Date | string | null }) {
  if (article.status === "published") return { label: "Publicado", accent: true };
  if (article.status === "scheduled") {
    const date = article.scheduledAt ? formatEdited(article.scheduledAt) : "";
    return { label: `Agendado — ${date}`, accent: true };
  }
  return { label: "Rascunho", accent: false };
}
    ...
    <DashboardLayout>
      <div className="mx-auto max-w-6xl pb-12">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-4">
          ...
          <div className="border border-black p-4"><p className="font-mono text-[10px] uppercase tracking-[0.12em] text-neutral-500">Agendados</p><p className="mt-3 text-3xl font-black tracking-[-0.07em]">{articles.filter((article) => article.status === "scheduled").length}</p></div>
        ...
        <section className="mt-11">
          ...
          {isLoading ? <p className="py-12 font-mono text-xs uppercase tracking-[0.13em]">A carregar artigos…</p> : articles.length ? <div className="divide-y divide-black border-y-2 border-black">{articles.map((article, index) => { const badge = statusBadge(article); return <div key={article.id} className="grid gap-3 py-5 sm:grid-cols-[55px_1fr_auto_auto] sm:items-center sm:gap-6"><span className="font-mono text-[11px] text-[#f0372f]">{String(index + 1).padStart(2, "0")}</span><button onClick={() => setLocation(`/redacao/${article.id}`)} className="group min-w-0 text-left"><span className="block break-words text-xl font-black tracking-[-0.055em] group-hover:underline">{article.title}</span><span className="mt-1 block text-xs text-neutral-500">{article.authorName} · atualizado a {formatEdited(article.updatedAt)}</span></button><span className={`w-fit border px-2 py-1 font-mono text-[9px] font-bold uppercase tracking-[0.12em] ${badge.accent ? "border-[#f0372f] text-[#f0372f]" : "border-black text-neutral-600"}`}>{badge.label}</span><div className="flex items-center justify-end gap-3"><ArrowUpRight className="hidden transition-transform group-hover:translate-x-1 sm:block" size={18} /><button type="button" disabled={article.status === "published" || deleteDraft.isPending} onClick={() => setDeleteTarget({ id: article.id, title: article.title })} className="flex h-8 w-8 items-center justify-center border border-transparent text-neutral-500 transition-colors hover:border-[#f0372f] hover:text-[#f0372f] disabled:cursor-not-allowed disabled:opacity-20" aria-label={`Apagar ${article.title}`}><Trash2 size={15} /></button></div></div>; })}</div> : <div className="border-y-2 border-black py-12"><FilePenLine size={24} className="mb-4 text-[#f0372f]" /><p className="text-xl font-black tracking-[-0.055em]">A primeira história ainda está por escrever.</p><Button onClick={() => createArticle.mutate({ title: newDraftTitle() })} variant="outline" className="mt-5 rounded-none border-black text-[10px] font-bold uppercase tracking-[0.12em]">Criar artigo</Button></div>}
        </section>
      </div>
      <AlertDialog open={Boolean(deleteTarget)} onOpenChange={(open) => { if (!open && !deleteDraft.isPending) setDeleteTarget(null); }}>
        <AlertDialogContent className="rounded-none border-2 border-black">
          <AlertDialogHeader><AlertDialogTitle>Apagar este artigo?</AlertDialogTitle><AlertDialogDescription>“{deleteTarget?.title}” será removido permanentemente, incluindo os seus capítulos e imagens. Esta ação não pode ser desfeita.</AlertDialogDescription></AlertDialogHeader>
          <AlertDialogFooter><AlertDialogCancel disabled={deleteDraft.isPending} className="rounded-none border-black">Cancelar</AlertDialogCancel><AlertDialogAction disabled={deleteDraft.isPending} onClick={() => { if (deleteTarget) deleteDraft.mutate({ id: deleteTarget.id }); }} className="rounded-none bg-[#f0372f] text-white hover:bg-black">{deleteDraft.isPending ? "A apagar…" : "Apagar"}</AlertDialogAction></AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </DashboardLayout>
  );
