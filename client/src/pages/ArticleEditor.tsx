...
import { ArrowLeft, CalendarClock, Check, ChevronDown, Eye, GripVertical, ImageUp, Plus, Save, Send, Trash2, Upload, X } from "lucide-react";
...
type PickableArticle = { id: number; title: string; status: "draft" | "scheduled" | "published" };
...
export default function ArticleEditor() {
  ...
  const scheduleArticle = trpc.editorial.manage.schedule.useMutation();
  ...
  useEffect(() => {
    ...
    setScheduleDate(article.scheduledAt ? new Date(article.scheduledAt).toISOString().slice(0, 10) : "");
  ...
  const readLabel = useMemo(() => `${sections.length} blocos · ${images.length}/50 imagens`, [sections.length, images.length]);
  ...
  async function saveAll(showToast = true) {
    ...
    if (!metadata.title.trim() || !metadata.authorName.trim()) { toast.error("Indique um título e o nome da autoria antes de guardar."); return false; }
    try {
      ...
      return true;
    } catch (error) { toast.error(error instanceof Error ? error.message : "Não foi possível guardar as alterações."); return false; }
  }

  // The tab is opened synchronously (inside the click) and pointed at the
  // preview once saving finishes — opening it after the awaited save would
  // risk the browser's popup blocker, since the click's user-activation
  // window can lapse during a slow save. The named target reuses the same
  // tab on repeat clicks instead of piling up new ones.
  async function handlePreview() {
    if (!article) return;
    const url = `/redacao/${article.id}/preview`;
    const tab = window.open("about:blank", `preview-${article.id}`);
    const saved = await saveAll(false);
    if (!saved) { tab?.close(); return; }
  ...
  async function handleSchedule() {
    ...
    } catch (error) { toast.error(error instanceof Error ? error.message : "Não foi possível agendar o artigo."); }
  }
  ...
  async function handleImageUpload(event: ChangeEvent<HTMLInputElement>, target: "cover" | "gallery" | "social") {
    ...
    if (target === "gallery") {
      ...
      if (remaining <= 0) { toast.error("Cada artigo suporta até 50 imagens na galeria. Use a multimédia para mais fotografias."); return; }
  ...
  return <DashboardLayout><div className="mx-auto max-w-6xl pb-12"><div className="flex flex-col gap-5 border-b-2 border-black pb-5 xl:flex-row xl:items-center xl:justify-between"><div><Link href="/redacao" className="inline-flex items-center gap-2 font-mono text-[10px] font-bold uppercase tracking-[0.12em] text-neutral-500 hover:text-black"><ArrowLeft size={14} /> Redação</Link><div className="mt-4 flex flex-wrap items-center gap-3"><h1 className="text-3xl font-black tracking-[-0.07em] sm:text-4xl">Editor de artigo</h1><span className={`border px-2 py-1 font-mono text-[9px] font-bold uppercase tracking-[0.12em] ${articleStatus !== "draft" ? "border-[#f0372f] text-[#f0372f]" : "border-black text-neutral-600"}`}>{articleStatus === "published" ? "Publicado" : articleStatus === "scheduled" ? `Agendado — ${article.scheduledAt ? new Date(article.scheduledAt).toLocaleDateString("pt-PT") : ""}` : "Rascunho"}</span></div><p className="mt-2 text-xs text-neutral-500">{readLabel}</p></div><div className="flex flex-wrap gap-2"><Button onClick={() => saveAll()} disabled={isSaving || uploading} variant="outline" className="h-10 rounded-none border-black text-[10px] font-bold uppercase tracking-[0.1em]"><Save size={14} /> {isSaving ? "A guardar…" : "Guardar"}</Button><Button onClick={() => void handlePreview()} disabled={isSaving || uploading} variant="outline" className="h-10 rounded-none border-black text-[10px] font-bold uppercase tracking-[0.1em]"><Eye size={14} /> Pré-visualizar</Button>{articleStatus !== "published" && <Button onClick={() => setDeleteDialogOpen(true)} disabled={isSaving || deleteDraft.isPending} variant="outline" className="h-10 rounded-none border-[#f0372f] text-[10px] font-bold uppercase tracking-[0.1em] text-[#f0372f] hover:bg-[#f0372f] hover:text-white"><Trash2 size={14} /> Apagar</Button>}<Button onClick={() => handlePublish(articleStatus !== "published")} disabled={isSaving || publishArticle.isPending} className="h-10 rounded-none bg-[#f0372f] text-[10px] font-bold uppercase tracking-[0.1em] text-white hover:bg-black"><Send size={14} /> {articleStatus === "published" ? "Retirar" : "Publicar agora"}</Button></div></div>
    ...
    <div className="mt-8 grid gap-8 xl:grid-cols-[minmax(0,1fr)_280px]"><div className="space-y-10"><section className="editor-panel"><div className="editor-panel-heading"><span>01</span><h2>Identidade</h2></div><div className="grid gap-5 sm:grid-cols-2"><div className="sm:col-span-2"><Label htmlFor="title">Título (destaque e notícias)</Label><Input id="title" value={metadata.title} onChange={(event) => setMetadata((current) => ({ ...current, title: event.target.value }))} className="editor-input text-xl font-bold tracking-[-0.04em]" placeholder="Título da história" /></div><div className="sm:col-span-2"><Label htmlFor="article-title">Título no artigo</Label><Input id="article-title" value={metadata.articleTitle ?? ""} onChange={(event) => setMetadata((current) => ({ ...current, articleTitle: event.target.value }))} className="editor-input text-xl font-bold tracking-[-0.04em]" placeholder="Por omissão, utiliza o título acima" /><p className="mt-2 font-mono text-[10px] leading-relaxed text-neutral-500">Deixe em branco para repetir o título de destaque dentro do artigo.</p></div><div className="sm:col-span-2"><Label htmlFor="deck">Subtítulo</Label><Textarea id="deck" value={metadata.deck ?? ""} onChange={(event) => setMetadata((current) => ({ ...current, deck: event.target.value }))} className="editor-input min-h-24" placeholder="Uma introdução que enquadra a história." /></div><div><Label htmlFor="author">Autoria</Label><Input id="author" value={metadata.authorName} onChange={(event) => setMetadata((current) => ({ ...current, authorName: event.target.value }))} className="editor-input" placeholder="Nome da autoria" /></div><div><Label htmlFor="slug">Endereço</Label><Input id="slug" value={metadata.slug} onChange={(event) => setMetadata((current) => ({ ...current, slug: event.target.value }))} className="editor-input font-mono text-xs" placeholder="slug-do-artigo" /></div></div></section>
      ...
      <aside className="space-y-6 xl:sticky xl:top-4 xl:self-start"><section className="border border-black p-5"><TagMultiSelect label="Tipo de carro" options={categories.filter((category) => category.kind === "tipo")} selectedIds={categoryIds.filter((id) => categories.some((category) => category.id === id && category.kind === "tipo"))} onChange={(tipoIds) => setCategoryIds((items) => [...items.filter((id) => !categories.some((category) => category.id === id && category.kind === "tipo")), ...tipoIds])} placeholder="Selecionar tipo…" /></section><section className="border border-black p-5"><TagMultiSelect label="Marca" options={categories.filter((category) => category.kind === "marca")} selectedIds={categoryIds.filter((id) => categories.some((category) => category.id === id && category.kind === "marca"))} onChange={(marcaIds) => setCategoryIds((items) => [...items.filter((id) => !categories.some((category) => category.id === id && category.kind === "marca")), ...marcaIds])} placeholder="Selecionar marca…" onCreate={handleCreateMarca} /></section><section className="border border-black p-5"><label className="flex cursor-pointer items-center gap-3 text-sm font-semibold"><Checkbox checked={metadata.isFeatured} onCheckedChange={(checked) => setMetadata((current) => ({ ...current, isFeatured: Boolean(checked) }))} /><span>Mostrar em destaque</span></label><p className="mt-3 text-xs leading-relaxed text-neutral-500">Os artigos destacados são priorizados na página inicial.</p></section><section className="border-2 border-black p-5">
        <p className="font-mono text-[10px] font-bold uppercase tracking-[0.12em] text-[#f0372f]">Publicação</p>
        <p className="mt-3 text-sm leading-relaxed text-neutral-600">
          {articleStatus === "published"
            ? "O artigo está visível ao público."
            : articleStatus === "scheduled"
              ? `Agendado para ${article.scheduledAt ? new Date(article.scheduledAt).toLocaleDateString("pt-PT") : ""} — fica invisível até lá.`
              : "O artigo fica invisível ao público enquanto estiver em rascunho."}
        </p>
        <Button onClick={() => handlePublish(articleStatus !== "published")} disabled={isSaving || publishArticle.isPending} className="mt-5 h-10 w-full rounded-none bg-black text-[10px] font-bold uppercase tracking-[0.11em] text-white hover:bg-[#f0372f]"><Check size={14} /> {articleStatus === "published" ? "Marcar como rascunho" : "Publicar no site"}</Button>
        {articleStatus === "scheduled" && <Button onClick={() => handlePublish(false)} disabled={isSaving || publishArticle.isPending} variant="outline" className="mt-2 h-10 w-full rounded-none border-black text-[10px] font-bold uppercase tracking-[0.11em]">Cancelar agendamento</Button>}

        {articleStatus !== "published" && (
          <div className="mt-5 border-t border-black pt-5">
            <Label htmlFor="schedule-date">Ou agendar para uma data</Label>
