...
type EditorMetadata = { title: string; slug: string; deck: string | null; authorName: string; coverImageUrl: string | null; coverImageCaption: string | null; seoTitle: string | null; seoDescription: string | null; socialImageUrl: string | null; isFeatured: boolean };

const emptyMetadata: EditorMetadata = { title: "", slug: "", deck: "", authorName: "", coverImageUrl: null, coverImageCaption: "", seoTitle: "", seoDescription: "", socialImageUrl: null, isFeatured: false };
...
export default function ArticleEditor() {
  ...
  useEffect(() => {
    ...
    setMetadata({ title: article.title, slug: article.slug, deck: article.deck, authorName: article.authorName, coverImageUrl: article.coverImageUrl, coverImageCaption: article.coverImageCaption, seoTitle: article.seoTitle, seoDescription: article.seoDescription, socialImageUrl: article.socialImageUrl, isFeatured: article.isFeatured });
  ...
  const readLabel = useMemo(() => `${sections.length} blocos · ${images.length}/3 imagens`, [sections.length, images.length]);
  ...
  async function saveAll(showToast = true) {
    ...
    try {
      await saveMetadata.mutateAsync({ id: article.id, ...metadata, title: metadata.title.trim(), slug: metadata.slug.trim() || metadata.title.trim() });
  ...
  async function handleImageUpload(event: ChangeEvent<HTMLInputElement>, target: "cover" | "gallery" | "social") {
    ...
    if (target === "gallery" && images.length >= 3) { toast.error("Cada artigo suporta até 3 imagens na galeria. Use a multimédia para mais fotografias."); return; }
    setUploading(true);
    try {
      ...
      toast.success(target === "cover" ? "Imagem de capa pronta a guardar." : target === "social" ? "Imagem de partilha pronta a guardar." : "Imagem adicionada à galeria. Guarde o artigo para persistir a ordem e a legenda.");
