...
export default function ArticleEditor() {
  ...
  const readLabel = useMemo(() => `${sections.length} blocos · ${images.length}/100 imagens`, [sections.length, images.length]);
  ...
  async function handleImageUpload(event: ChangeEvent<HTMLInputElement>, target: "cover" | "gallery" | "social") {
    ...
    if (target === "gallery" && images.length >= 100) { toast.error("Cada galeria suporta até 100 imagens."); return; }
    setUploading(true);
    try {
