        ...
        <div className="article-copy">
          {article.sections.map((section, index) => {
            ...
            if (section.type === "chapter") {
              return <section key={section.id} className="article-chapter"><div className="article-chapter-heading mb-4"><span className="article-chapter-marker" /><h2>{section.heading}</h2></div>{section.body && <p className={isOpening ? "drop-cap" : ""}>{section.body}</p>}{section.caption && <p className="article-caption">{section.caption}</p>}</section>;
            }
...
export default function Article() {
  ...
  if (isLoading) return <div className="min-h-screen bg-white"><EditorialHeader categories={categories} /><div className="editorial-shell py-28 font-mono text-xs uppercase tracking-[0.15em]">A preparar leitura…</div></div>;
  if (!article) return <div className="min-h-screen bg-white"><EditorialHeader categories={categories} /><main className="editorial-shell py-28"><p className="text-[11px] font-bold uppercase tracking-[0.15em] text-[#f0372f]">404 / Artigo indisponível</p><h1 className="mt-4 max-w-xl text-5xl font-black tracking-[-0.07em]">Esta estrada não tem história.</h1><Link href="/" className="mt-8 inline-flex items-center gap-2 border-b-2 border-black pb-1 text-sm font-bold uppercase tracking-[0.1em]"><ArrowLeft size={16} /> Voltar ao início</Link></main><EditorialFooter /></div>;

  const readTime = estimateReadingMinutes(article.sections);
  return (
    <div className="min-h-screen bg-white">
      <EditorialHeader categories={categories} />
      <main>
